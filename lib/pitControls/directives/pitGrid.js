...
var PitGridController = function($scope, $element, $attrs, $q, $http, $log, $timeout, $compile, utilities){
  ...
  $scope.getClassNames = function(row){
    ...
    classes["pit-grid-row-hover"] = !!row.pitGridRowHover;
    classes["pit-grid-row-selected"] = !!row.pitGridRowSelected
  ...
  $scope.$on('pitGridDomLinked', function(){
    ...
    $scope.$watch($attrs.pitGridDataSource + '.length', function(newVal){
      rowWatchHandler();
    });
      ...
      function processTemplate(htmlTemplate){
        ...
        htmlTemplate = $(
          '<div>'+
            ...
            '<div class="pit-grid-container" style="overflow:auto; height:'+(angular.isNumber($scope.gridHeight) ? $scope.gridHeight + 'px' : $scope.gridHeight)+'; position:relative;">' + htmlTemplate + '</div>'+
          '</div>'
        );
        ...
        htmlTemplate.find('table tbody').each(function(i,tbody){
          ...
          if(angular.isDefined(attrs.pitGridRowHoverHighlight)){
            rowAttrs['ng-mouseenter'] = 'row.pitGridRowHover = true';
            rowAttrs['ng-mouseleave'] = 'row.pitGridRowHover = false';
          }
