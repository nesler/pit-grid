...
var PitGridController = function($scope, $element, $attrs, $q, $http, $log, $timeout, $compile, utilities){
  ...
  $scope.getClassNames = function(row, index){
    row.pitGridClassNames = {
      'pit-grid-row': true,
      'pit-grid-row-hover' : !!row.pitGridRowHover,
      'pit-grid-row-selected' : !!row.pitGridRowSelected
    }

    if(customGroupingColumns && index > 0){
...
function addFixedColumnMarkup(htmlTemplate, $scope){
  ...
  htmlTemplate.find('.mainTable').wrap('<div style="overflow: auto; height:'+$scope.gridHeight + '' + $scope.gridHeightUnit + ';"/>');
      ...
      function processTemplate(htmlTemplate){
        ...
        htmlTemplate.find('table tbody').each(function(i,tbody){
          ...
          if(angular.isDefined(attrs.pitGridRowHoverHighlight)){
            rowAttrs['ng-mouseenter'] = 'row.pitGridRowHover = true; getClassNames(row);';
            rowAttrs['ng-mouseleave'] = 'row.pitGridRowHover = false; getClassNames(row);';
          }
