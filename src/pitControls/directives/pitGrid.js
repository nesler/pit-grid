...
pitDirectives.directive('pitGrid', ['$http','$compile','$log','utilities',function($http, $compile, $log, utilities){
  var PitGridController = function($scope, $element, $attrs, $q, $http, $log, $timeout, $compile, utilities){
    ...
    function renderChuncked(){
      ...
      var loader = $compile(
                    '<div pit-overlay style="cursor:wait;">'+
                      '<div pit-progress-indicator="renderProgress" pit-progress-indicator-max-value="'+max+'" style="width:70%; margin-left:15%; margin-top:'+$scope.gridHeight/2 + '' + $scope.gridHeightUnit +'; position:absolute; z-index:2;"></div>'+
                    '</div>'
                    )($scope);
      ...
      function processTemplate(htmlTemplate){
        ...
        htmlTemplate = $(
          '<div>'+
            ...
            '<div class="pit-grid-container" style="overflow:auto; height:'+$scope.gridHeight + '' + $scope.gridHeightUnit+';">' + htmlTemplate + '</div>'+
          '</div>'
        );
        ...
        if(angular.isDefined($scope.isLoading)){
          ...
          htmlTemplate.find('.pit-grid-container').append(tblLoader)
        }
